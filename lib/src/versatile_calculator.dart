...
class VersatileCalculator extends HookConsumerWidget {
  // double doDouble = 0.0;
  final List<String> integersCalculatorList = [
    'C',
    'CE',
    '%',
    '/',
  ...
  final List<Color> integersCalculatorColorList = [
    Colors.blue[800]!,
    Colors.blue[800]!,
    Colors.orange[800]!,
    Colors.orange[800]!,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    bool checkLastBadValue(String valString) {
      var lastStringItem =
          valString.substring(valString.length - 1, valString.length);
      if (lastStringItem == '+' ||
          lastStringItem == '-' ||
          lastStringItem == '/' ||
          lastStringItem == 'x' ||
    ...
    String calculateInstant(String valString, String otherStringIncludingSign) {
      ...
      if (valString.contains('+') ||
          valString.contains('-') ||
          valString.contains('/') ||
          valString.contains('x') ||
          valString.contains('.') ||
          otherStringIncludingSign == '=') {
        Parser p = Parser();
        ...
        // debugPrint('$lastChar');
        if (otherStringIncludingSign == '+' ||
            otherStringIncludingSign == '-' ||
            otherStringIncludingSign == '/' ||
            otherStringIncludingSign == 'x' ||
            otherStringIncludingSign == '=') {
          calculateHistoryList.value
              .add('$tempString = ${eval.toStringAsFixed(2)}');
      ...
      switch (index) {
        ...
        case 2:
          return () {
            ...
            if (checkLastBadValue(valueController.text) == false) {
              Parser p = Parser();
              Expression exp =
                  p.parse('${valueController.text.replaceAll('x', '*')}');
              ContextModel cm = ContextModel();
              double eval = exp.evaluate(EvaluationType.REAL, cm);
              String evalPerCent = (eval / 100).toStringAsFixed(2);
              calculateHistoryList.value
                  .add('${valueController.text}/100 = $eval ');
        ...
        case 3:
          return () {
            if (checkLastBadValue(valueController.text) == false) {
              // valueController.text = valueController.text + '/';
              valueController.text =
                  calculateInstant(valueController.text, '/') + '/';
              // displayString.value = valueController.text;
            }
          };
        case 4:
        ...
        case 7:
          return () {
            if (checkLastBadValue(valueController.text) == false) {
              valueController.text =
                  calculateInstant(valueController.text, 'x') + 'x';
                        ...
                        child: Container(
                          ...
                          child: ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                return Text(
                                    '${calculateHistoryList.value[index]}');
                              },
                      ...
                      child: Container(
                        child: GridView.count(
                            ...
                            children: List.generate(
                                integersCalculatorList.length, (index) {
                              return InkWell(
                                onTap: () {
                                  getCalculatorFunctionFromIndex(
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                      ...
                                      color:
                                          integersCalculatorColorList[index]),
                                  child: Center(
                                      child: Text(
                                    '${integersCalculatorList[index]}',
                                    style: TextStyle(
                                        color: Colors.white,
